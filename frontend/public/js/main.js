...
const COLOURS = [
  getCssVar('--read-particle-1'),
  getCssVar('--read-particle-2'),
  getCssVar('--read-particle-3'),
  getCssVar('--read-particle-4'),
  getCssVar('--read-particle-5'),
  getCssVar('--read-particle-6'),
  getCssVar('--read-particle-7')
].filter(Boolean)
const smokeRgb = getCssVar('--read-smoke-rgb')
...
class Particle {
  ...
  init(x, y, radius) {
    ...
    this.drag = 0.92
    this.color = getCssVar('--read-color-fff')
  ...
  draw(ctx) {
    ...
    ctx.fillStyle = `rgba(${smokeRgb}, ${1 - (this.radius - 20) / 40})`
    ctx.fill()
  }
}
